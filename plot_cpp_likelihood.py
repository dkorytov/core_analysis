...
plt.figure()
plt.plot(np.log10(mi_bins),mi_lkhd)
plt.xlabel('M_infall')
...
if(rd_bins.size > 2):
    plt.figure()
    plt.pcolor(np.log10(mi_bins),rd_bins,mi_rd_lkhd.T,norm=LogNorm(),cmap='nipy_spectral_r')
    cb = plt.colorbar()
    cb.set_label('X^2')
    # plt.xscale('log')
    plt.xlabel('M_infall')
    plt.ylabel('R_disrupt')
    plt.xlim((np.log10(np.min(mi_bins)),np.log10(np.max(mi_bins))))
...
if(fit_r_merger):
    plt.figure()
    plt.pcolor(np.log10(mi_bins),rm_bins,mi_rm_lkhd.T,norm=LogNorm(),cmap='nipy_spectral_r')
    cb = plt.colorbar()
    cb.set_label('X^2')
    # plt.xscale('log')
    plt.xlabel('M_infall')
