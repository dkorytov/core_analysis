...
expected_comov_abundance=0.000130578969889

# Two elements for m200c and m200m
zmrh5_locs=("/home/dkorytov/phys/Ngal_sdss/data/rad_profile_mstar-1_wmap7_simet_mean3/result/type1_weight1_mag1_clr1_result.hdf5" "/home/dkorytov/phys/Ngal_sdss/data/rad_profile_mstar-1_wmap7_simet_crit3/result/type1_weight1_mag1_clr1_result.hdf5")
cluster_locs=("tmp_hdf5/clusters_OR_M200m.tmp.hdf5" "tmp_hdf5/clusters_OR.hdf5")
...
for cluster_i in 0 1;do
	...
	for cost_i in 0 1; do
	    param_fname="params/cfn/simet/mstar-1/${cluster_types[$cluster_i]}/${cost_types[$cost_i]}a_${model_types[$model_i]}.param"
	    # echo ${cluster_i} ${model_i} ${cost_i}
	    param_dic=`echo "${param_fname}" | sed "s@a_${model_types[$model_i]}.param@@g"`
	    echo $param_dic
	    echo $param_fname
	    mkdir -p $param_dic
