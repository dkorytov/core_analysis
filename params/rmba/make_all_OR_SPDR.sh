...
zmrh5_locs=( '/data/a/cpac/dkorytov/data/Ngal_sdss_old/data/rad_profile_mstar@mstarval@_wmap7_spider_mean_bcg/result/type1_weight1_mag1_clr1_result.hdf5'
	   '/data/a/cpac/dkorytov/data/Ngal_sdss_old/data/rad_profile_mstar@mstarval@_wmap7_spider_crit_bcg/result/type1_weight1_mag1_clr1_result.hdf5')
...
for mstar in "${mstars[@]}"; do
	...
	for model_i in 0 1 2 3; do 
		...
		param_fname="params/rmba/auto/${script_name}/${cluster_types[$cluster_i]}/mstar${mstar}/${cost_types[$cost_i]}${prefix}_${model_types[$model_i]}.param"
		dir=${param_fname%/*}
		...
		if [ $make_param = true ]; then
		    ...
		    if { [ $mstar == -1 ] || [ $mstar == -0.5 ]; } || { [ $model_i == 0 ] && { [ $mstar == 0.5 ]; }; }
		    then
		    ...
		    ## have at least 5 in a mass bin for the cost calculation
		    sed -i "s/cost_min_cluster_num  0/cost_min_cluster_num 5/g" $param_fname
		fi
	    done
	done
    done
