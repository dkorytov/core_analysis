...
for mstar in "${mstars[@]}"; do
	...
	for model_i in 0 1 2 3; do 
	    for cost_i in 0; do
		prefix='OR'
		param_fname="params/rmba/simet/${cluster_types[$cluster_i]}/mstar${mstar}/${cost_types[$cost_i]}${prefix}_${model_types[$model_i]}.1kbin.param"
		dir=${param_fname%/*}
		...
		if [ $make_param = true ]; then
		    ...
		    if { [ $mstar == -1 ] || [ $mstar == -0.5 ]; } || { [ $model_i == 0 ] && { [ $mstar == 0 ] || [ $mstar == 0.5 ]; }; } then
		    ...
		    echo "cluster_bin_count_limit 1000" >>  $param_fname
		fi
	    done
	done
    done
