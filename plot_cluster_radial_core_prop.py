...
def plot_profile(clusters):
    ...
    color_map_r = plt.cm.cool_r
    colors = color_map(np.linspace(0, 1, 25))
    ...
    sm._A = []
    plt.figure()
    for i, m_cut in enumerate(np.logspace(11, 13, 25)):
    ...
    plt.yscale('log')
    plt.ylabel('Galaxy Density [R$_{200m}$$^{-3}$]')
    plt.xlabel('r/R$_{200m}')
    plt.tight_layout()
    print('===')
    plt.figure()
    ...
    cb.set_label('M$_{infall}$')
    plt.yscale('log')
    plt.ylabel('Galaxy Surface Density [R$_{200m}$$^{-2}$]')
    plt.xlabel('r/R$_{200m}')
    plt.tight_layout()


    print("\n\n")
    slct_m_infall = clusters.core_m > 1e8

    colors = color_map_r(np.linspace(0,1,25))
    vmin = 10**-2
    vmax = 0.3
    sm = plt.cm.ScalarMappable(cmap=color_map_r, norm=clr.LogNorm(vmin=vmin, vmax=vmax))
    sm._A = []
    plt.figure()
    for i, r_cut in enumerate(np.logspace(np.log10(vmin), np.log10(vmax), 25)):
    ...
    for i, r_cut in enumerate(np.logspace(np.log10(vmin), np.log10(vmax), 25)):
        ...
        label = r'M$_{{disrupt}} <$ {:.2e}, reduction: {:.2e}'.format(r_cut, np.float(np.sum(h))/np.sum(h2d))
        plt.plot(dtk.bins_avg(r_bins), h/r_bins_area/cluster_m_cut_count, '-', label=label, color=colors[i])
        print(label)
    plt.yscale('log')
    cb = plt.colorbar(sm)
    ...
    for i, r_merger in enumerate(np.logspace(np.log10(vmin), np.log10(vmax), 10)):
        ...
        label = r'R$_{{merge}} <$ {:.2e}, reduction: {:.2e}'.format(r_merger, np.float(cluster_m_cut_count)/np.sum(h2d))
        print(label)
        ax_2d.plot(dtk.bins_avg(r_bins), h_2d/r_bins_volume/cluster_m_cut_count, '-', label=label, color=colors[i])
    ...
    for fig, ax in [(f_2d, ax_2d), (f_3d, ax_3d)]:
        ...
        ax.set_ylabel('Galaxy Surface Density [R$_{200m}$$^{-2}$]')
...
if __name__ == "__main__":
    plot_cluster_radial_core_prop("tmp_hdf5/clusters_OR.crit.r10.hdf5")
    # plot_cluster_radial_core_prop("tmp_hdf5/clusters_OR_M200m.hdf5")
